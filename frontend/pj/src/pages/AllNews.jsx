...
//   // ✅ Fetch reactions for one news (likes/dislikes)
//   const fetchReactions = async (newsId) => {
//     try {
//       const res = await fetch(`${API_BASE}/news/${newsId}/reactions`, {
//         credentials: "include",
...
//             userAction: data.data.userAction ?? "NONE",
//           },
//         }));
//       }
//     } catch (err) {
...
//   const toggleComments = (newsId) => {
//     setShowComments((prev) => {
//       const visible = !prev[newsId];
//       if (visible && !commentsMap[newsId]) fetchComments(newsId);
//       return { ...prev, [newsId]: visible };
//     });
//   };
...
//   // ✅ Initial load - fetch all news, then reactions for each news
//   useEffect(() => {
//   fetchAllNews();            // 1️⃣ Get all news
// }, []);

// useEffect(() => {
//   if (newsList.length > 0) { // 2️⃣ Then load reactions for each
//     newsList.forEach((n) => fetchReactions(n.id));
//   }
// }, [newsList]);
...
import { useAllNewsQuery, useProfileQuery } from "../features/api/newsApi";
...
const AllNews = () => {
  ...
  const { data: newsResponse, isLoading } = useAllNewsQuery();
  const newsList = newsResponse?.data || [];

  if (isLoading) {
    return <p className="text-center text-gray-400 mt-10">Loading news...</p>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-5 md:p-10">
      <div className="max-w-6xl mx-auto">
        ...
        {newsList.map((news) => (
          <NewsCard key={news.id} news={news} user={user} />
        ))}
      </div>
    </div>
  );
